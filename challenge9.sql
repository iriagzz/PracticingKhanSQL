...
SELECT SB, Loser, Loser_Pts, Winner, Winner_Pts FROM superbowls WHERE Loser_Pts > 10 AND Winner_Pts > 20;

SELECT SB, Loser, Loser_Pts, Winner, Winner_Pts FROM superbowls WHERE Loser_Pts < 10 or Loser_Pts > 30;


SELECT Winner,date, MAX(Winner_Pts) AS Winner_max_pts FROM superbowls;

SELECT Winner,date, MIN(Winner_Pts) AS Winner_min_pts FROM superbowls;

SELECT Winner, AVG(Winner_Pts) AS Winner_avg_pts FROM superbowls GROUP BY Winner ORDER BY Winner_avg_pts;

SELECT MVP, COUNT(*) AS MVP_times FROM superbowls GROUP BY MVP HAVING MVP_times ORDER BY MVP_times;

SELECT Winner_pts, COUNT(*) AS Quantity,
CASE
    WHEN Winner_pts > 30 THEN "Lavada"
    WHEN Winner_pts > 20 THEN "Bom Jogo"
    WHEN Winner_pts > 10 THEN "foi por pouco"
    ELSE "só um chute?"
END AS "champion"
FROM superbowls GROUP BY champion;
